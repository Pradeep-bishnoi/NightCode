  ...
  return new Promise<{ token: string }>((resolve, reject) => {
    const server = Bun.serve({
      ...
      async fetch(req) {
        ...
        try {
          // IMPORTANT:
          // This MUST be the exact same redirect URI
          ...
          if (!tokenData.access_token) {
            throw new Error(
              "No access token returned by Clerk",
            );
          }
    ...
    // IMPORTANT:
    // Clerk will redirect back to this exact local server.
    const redirectUri =
    ...
    console.log(
      `OAuth callback listening on ${redirectUri}`,
    );
